...
2. ✅ **Canonical memory path** — baseline endpoint now uses `_validate_mutable_memory` so canonical-system users are validated and 404'd from the correct store
3. ✅ **Behavioral tests** — replaced regex/source-level checks with `MemoryDB` instantiation + mocked injection logic tests
4. ⏳ **Downstream invalidation** — noted for follow-up; visibility endpoint uses `submit_with_context(postprocess_executor, update_personas_async, uid)` which could also trigger on baseline toggle if persona relevance is affected

**Status:** Phase IV — PR feedback addressed, pushed for maintainer re-review
